...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
 	...
 	if(cmd == REQUEST_SYSCALL_INTERCEPT){
		...
		//intercept an intercepeted system call
		...
		//de-intercept an unintercepted systemcall
		...
		//syscall not intercepted; can't be monitored either
		if(table[syscall].intercepted == 0){
			return -EINVAL;
		}
